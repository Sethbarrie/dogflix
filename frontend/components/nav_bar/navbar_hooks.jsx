...
const NavBarHooks = ({user = {}, postSession, deleteSession, updateAutoplay, history, location}) => {
    ...
    if(!emptyObject(user)){
        return(
            <header className='navbar' className={location.pathname === '/browse' ? 'monster-z-index navbar' : 'navbar'} id={location.pathname === '/my-list' ? 'signedin-navbar-small-z-index' : 'signedin-navbar'}>
